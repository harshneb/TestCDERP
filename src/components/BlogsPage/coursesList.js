...
const CoursesList = ({ initialCategory = 'all' }) => {
  ...
  return (
    <>
      <div className="relative min-h-screen p-6" style={{background: '#0a2a43'}}>
        ...
        {/* Header */}
          <div className="mb-12">
            <h1 className="text-3xl font-bold text-white mb-4 bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent text-left">
              Start Your Learning Journey
          </h1>
            <div className="w-64 h-1 mb-4 rounded-full bg-gradient-to-r from-blue-700 to-cyan-400"></div>
          ...
          {categories.map((category) => (
            <button
              ...
              onClick={() => setSelectedCategory(category.id)}
                className={`px-6 py-3 rounded-full font-medium transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-400
                  ${selectedCategory === category.id
                    ? 'bg-gradient-to-r from-blue-500 to-cyan-400 text-white border-4 border-white shadow-lg' 
        ...
        {/* Learning Topics Grid */}
          ...
          {filteredTopics.map((topic, index) => {
            ...
            const cardContent = (
              <div
                key={topic.name}
                className="flex items-center gap-3 bg-white border border-gray-200 rounded-lg shadow-sm px-5 py-1 min-w-[220px] max-w-full min-h-[40px] mb-2 hover:scale-105 hover:shadow-xl transition-transform duration-200"
              >
                <IconComponent className="w-7 h-7 text-blue-900" />
                <span className="font-semibold text-gray-800 text-base">{topic.name}</span>
              </div>
            );
            if (isSap) {
              return (
